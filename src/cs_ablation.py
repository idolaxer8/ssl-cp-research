...
def main():
    ...
    for cs in args.cal_sizes:
        ...
        for trial in range(args.n_trials):
            ...
            # --- FCP (768d) ---
            c, s = run_fcp(X_cal, y_cal, X_test, y_test, all_classes, args.ncm, args.alpha)
            all_results[cs]["FCP"]["cov"].append(c)
            all_results[cs]["FCP"]["sz"].append(s)

            # --- FCP+MS-CS (768d) ---
            c, s = run_fcp_mscs(X_cal, y_cal, X_test, y_test, X_unlabeled,
            ...
            # --- FCP+PCA ---
            c, s = run_fcp(X_cal_pca, y_cal, X_test_pca, y_test, all_classes, args.ncm, args.alpha)
            all_results[cs]["FCP+PCA"]["cov"].append(c)
            all_results[cs]["FCP+PCA"]["sz"].append(s)

            # --- FCP+PCA+MS-CS (clusters in PCA space) ---
            c, s = run_fcp_mscs(X_cal_pca, y_cal, X_test_pca, y_test, X_u_pca,
                                all_classes, args.ncm, args.alpha,
                                args.mscs_lambda, args.mscs_n_clusters, args.mscs_tau_mult)
            ...
            # --- FCP+AE ---
            c, s = run_fcp(X_cal_ae, y_cal, X_test_ae, y_test, all_classes, args.ncm, args.alpha)
            all_results[cs]["FCP+AE"]["cov"].append(c)
            all_results[cs]["FCP+AE"]["sz"].append(s)

            # --- FCP+AE+MS-CS (clusters in AE space) ---
            c, s = run_fcp_mscs(X_cal_ae, y_cal, X_test_ae, y_test, X_u_ae,
                                all_classes, args.ncm, args.alpha,
                                args.mscs_lambda, args.mscs_n_clusters, args.mscs_tau_mult)
    ...
    print(f"{'='*90}")
    hdr = f"{'Cal':>6}  {'Method':>20}  {'Coverage':>18}  {'Set Size':>18}"
    print(hdr)
    print("-" * 90)
    ...
    for cs in args.cal_sizes:
        ...
        for m_name in methods:
            ...
            line = f"{prefix}  {m_name:>20}  {cov_str:>18}  {sz_str:>18}"
            print(line)
