...
def compare_cp_methods(
    ...
    * Split CP: split the same ``cal_size`` budget into
    ...
    if args.ncm != "softmax":
        print(f"NCM (Full CP): {args.ncm} (k={args.k})")
    else:
        print(f"Full CP: skipped (softmax has no hypothetical update)")
    print(f"NCM (CV+): {ncm_cv_type} (k={args.k})")
...
def main():
    ...
    # 3b. Optional: Compare different k values
    if args.compare_k:
        print(f"\n[3b] Comparing performance across different k values...")
        compare_k_values(X_cal, y_cal, X_test, y_test, args, args.output_dir)

    # 3c. Optional: Compare Full CP vs CV+ vs Split CP
    if args.compare_cp_methods:
        print(f"\n[3c] Comparing Full CP vs CV+ vs Split CP across calibration sizes...")
