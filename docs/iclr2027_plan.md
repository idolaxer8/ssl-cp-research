...
**Housekeeping actions:** DONE 08-18 — core branches merged, archive notes in
`findings.md` §12, transform-selection was already committed. Remaining for
wk 08-24: confirm every headline number has a reproduction script on main.

### 3b. Code reduction toward the compact publishable version

**Round 1 EXECUTED 2026-08-18** (after the worktree merges): `src/` cut from
62 to 45 scripts. Archived (verbatim `git mv` to `src/archive/`, README
updated, CLAUDE.md active list rewritten by claim area): the entire 08-10
watch list (12 scripts; the 4 HOLD conditions retired — their findings-§10
items are from the stale 2026-05 plan, superseded by this plan) + 5 closed
lines (`ica_negent_pilot`, `ae_lowpr_pilot`, `transform_selection_pilot` +
`plot_framework_summary` (C4 descoped), `semicp_experiment` (superseded by
`g3_semisup_experiment`)). `cluster/run_small_cal_sweep.sh` path updated.

**Round 2 (wk 08-24, with the method freeze) — the publishable core.** The
paper artifact is a SUBSET, not a rewrite; target ~20 files:

```
Core library:      conformal_prediction.py, split_cp_baselines.py,
                   exchangeable_features.py (qe + pool transforms), mscs_gpu.py
Data prep:         download_datasets.py, extract_features.py, carve_unlabeled.py
Entry point:       run_conformal_experiment.py, exchangeable_fcp_experiment.py
Per-table drivers: g3_semisup_experiment.py        (C1 baselines table)
                   fca_family_cluster_experiment.py (C2 NCM table)
                   d1_softmax_ablation.py           (C2 prototype_cosine row)
                   transform_control_experiment.py  (C3 transform/qe menu)
                   backbone_dwt_experiment.py       (C5 backbone table)
Theory validation: d1_empirical_check.py, wt_chainfree_diagnostics.py
Regime map (C4):   dwt_gate_constants.py, spectral_band_diagnostic.py
Tests:             tests/ (GPU parity + exchangeability oracles)
```

Round-2 actions: (a) archive the remaining one-off plot scripts after their
figures are regenerated by the drivers; (b) decide `pca_experiment.py` /
`fca_ablation_cluster_experiment.py` (fold into transform_control / fca_family
or archive); (c) theory-instrument scripts beyond the two validators move to
a `src/theory/` folder or stay archived with the D3 layer (decision D-1);
(d) the paper repo ships the subset + one `reproduce.sh` per table.
